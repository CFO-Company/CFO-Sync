    ...
    try {
        ...
        foreach ($token in $tokens) {
            $safeTokens += [ordered]@{
                ...
                allowed_server_versions = @($token.allowed_server_versions)
...
function Write-MarkdownReport {
    ...
    foreach ($token in $Report.access.tokens) {
        ...
        $serverVersions = (($token.allowed_server_versions | ForEach-Object { Convert-ToSafeString $_ }) -join ", ")
        if (-not $serverVersions) {
            $serverVersions = "(nenhuma)"
        }
        $lines += "- Token name=$($token.name), token_present=$($token.token_present), can_manage_secrets=$($token.can_manage_secrets), can_select_server_version=$($token.can_select_server_version), platforms=$platforms, allowed_server_versions=$serverVersions"
    }
