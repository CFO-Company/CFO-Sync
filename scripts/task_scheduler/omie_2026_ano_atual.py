...
def _resolve_period() -> tuple[str, str]:
    ...
    today = date.today()
    end = today if today <= year_end else year_end
    if end < year_start:
        raise ValueError(
            f"Data atual ({today.isoformat()}) ainda nao alcancou o ano-alvo {TARGET_YEAR}."
        )
    return year_start.isoformat(), end.isoformat()
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Runner da Omie 2026 para ano atual (01/01/2026 ate hoje).",
    )
    parser.add_argument(
        "--log-dir",
