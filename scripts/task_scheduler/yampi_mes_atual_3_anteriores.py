...
def _build_logger(log_dir: Path) -> tuple[logging.Logger, Path]:
    ...
    log_path = log_dir / f"yampi_{date.today().isoformat()}.log"
    run_id = datetime.now().strftime("%Y%m%d_%H%M%S_%f")

    logger = logging.getLogger(f"cfo_sync.automation.yampi.{run_id}")
...
def _run(log_dir: Path) -> int:
    ...
    logger.info(
        "RUN_START platform=yampi python=%s log=%s periodo=%s..%s",
        sys.version.split()[0],
        str(log_path),
        start_date,
        end_date,
    ...
    platform = next((item for item in config.platforms if item.key == "yampi"), None)
    if platform is None:
        raise ValueError("Plataforma 'yampi' nao configurada no app_config.")

    resources = {resource.name for resource in platform.resources}
    if RESOURCE_NAME not in resources:
        raise ValueError(
            f"Recurso '{RESOURCE_NAME}' nao encontrado na plataforma yampi. Recursos atuais: {sorted(resources)}"
        )

    logger.info(
        "CLIENTES_YAMPI total=%s clientes=%s",
        len(platform.clients),
        ", ".join(platform.clients),
    )

    pipeline = SyncPipeline(config=config)
    if "yampi" not in pipeline.connectors:
        raise ValueError("Conector 'yampi' nao disponivel nesta instalacao.")
    ...
    for client in platform.clients:
        ...
        logger.info(
            "TASK_START platform=yampi cliente=%s recurso=%s",
            client,
            RESOURCE_NAME,
        )
        try:
            ...
            logger.info(
                "TASK_OK platform=yampi cliente=%s recurso=%s linhas=%s tempo=%.2fs",
                client,
                RESOURCE_NAME,
                exported,
                elapsed,
            ...
            failed_tasks += 1
            logger.error(
                "TASK_ERRO platform=yampi cliente=%s recurso=%s tempo=%.2fs msg=%s",
                client,
                RESOURCE_NAME,
                elapsed,
                error,
    ...
    logger.info(
        "RUN_END platform=yampi status=%s tarefas=%s falhas=%s linhas=%s tempo_total=%.2fs",
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Runner da Yampi para mes atual + 3 meses anteriores.",
    )
    parser.add_argument(
        "--log-dir",
