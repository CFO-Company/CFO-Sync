...
def _resolve_resource_name(platform, preferred: str) -> str:
    resource_names = [resource.name for resource in platform.resources]
    if preferred in resource_names:
        return preferred
    if not resource_names:
        raise ValueError(f"Nenhum recurso configurado para a plataforma {PLATFORM_KEY}.")
    return resource_names[0]


def _run(log_dir: Path, preferred_resource: str) -> int:
    ...
    resource_name = _resolve_resource_name(platform, preferred_resource)
    if resource_name != preferred_resource:
        logger.warning(
            "Recurso preferido '%s' nao encontrado. Usando '%s'.",
            preferred_resource,
            resource_name,
        )
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Runner do TikTok Shop para mes atual + 3 meses anteriores.",
    )
    parser.add_argument(
        "--log-dir",
    ...
    parser.add_argument(
        ...
        help=f"Recurso da plataforma (padrao: {DEFAULT_RESOURCE_NAME}).",
    )
    return parser
...
def main() -> int:
    ...
    try:
        return _run(log_dir=log_dir, preferred_resource=str(args.resource).strip())
    except Exception as error:  # noqa: BLE001
